...
public class N2NetworkEventHandler {
    ...
    public static <REQ extends IMessage, REPLY extends IMessage> void registerDoubleSidedMessageHandler(
    ...
    public static void sendPacketsToPlayer(List<N2Packet> packets, EntityPlayerMP player) {
        for (int i = 0; i < packets.size(); i++) sendPacketToPlayer(packets.get(i), player);
    }
    ...
    public static void sendPacketsToAllPlayers(List<N2Packet> packets) {
        for (int i = 0; i < packets.size(); i++) sendPacketToAllPlayers(packets.get(i));
    }
    ...
    public static void sendPacketsToServer(List<N2Packet> packets) {
        for (int i = 0; i < packets.size(); i++) sendPacketToServer(packets.get(i));
    }
    ...
    public static void sendPacketsToPlayersAround(List<N2Packet> packets, TargetPoint point) {
        for (int i = 0; i < packets.size(); i++) sendPacketToPlayersAround(packets.get(i), point);
    }
    ...
    public static void sendPacketsToPlayersInDimension(List<N2Packet> packets, int dimensionID) {
        for (int i = 0; i < packets.size(); i++) sendPacketToPlayersInDimension(packets.get(i), dimensionID);
    }
