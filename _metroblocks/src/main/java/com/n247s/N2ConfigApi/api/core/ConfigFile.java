...
public abstract class ConfigFile extends ConfigSectionCollection implements Cloneable, Serializable {
    ...
    private ConfigFile(String sectionName, String[] description, String fileName, ProxySide proxySide,
            boolean setExtraSpaces, boolean isVirtual) {
    ...
    public final void regenerateConfigFile(List<String> invalidSectionNames) {
        ...
        if (invalidSectionNames != null && !invalidSectionNames.isEmpty()) {
            try {
                File tempFile = ConfigHandler.generateSingleFileFromConfigFile(
                        this,
                        FileType.Temp,
                        N2ConfigApi.getTempDir(ConfigHandler.getFileFromConfigFile(this).getParentFile()));
                File configFile = ConfigHandler.generateSingleFileFromConfigFile(
                        this,
                        FileType.Original,
                        ConfigHandler.getFileFromConfigFile(this).getParentFile());
                ...
                if (sectionCount != this.AbsoluteSubSectionMap.size()) {
                    ...
                    File backUpFile = ConfigHandler.generateSingleFileFromConfigFile(
                            this,
                            ConfigHandler.FileType.BackUp,
                            N2ConfigApi.getTempDir(ConfigHandler.getFileFromConfigFile(this).getParentFile()));
                    ConfigHandler.copyFile(tempFile, backUpFile);
                    ConfigHandler.generateSingleFileFromConfigFile(
                            this,
                            ConfigHandler.FileType.Original,
                            ConfigHandler.getFileFromConfigFile(this).getParentFile());
                }
                closeWriter(writer);
                tempFile.delete();
    ...
    /**
     * This will check if the existing configuration file contains all registered sections. If this isn't the case it
    ...
    public final List checkConfigFile() {
        ...
        try {
            ...
            while ((readedLine = reader.readLine()) != null) {
                if (readedLine.startsWith("#") || readedLine.isEmpty()) continue;
                ...
                if (readedLine.contains(";")) {
                    for (int i = 0; i < invalidSectionList.size(); i++) {
                        ...
                        try {
                            if (invalidSectionList.contains(
                                    readedLine.substring(readedLine.indexOf(firstLetter), readedLine.indexOf(";")))) {
                                invalidSectionList.remove(
                                        readedLine.substring(readedLine.indexOf(firstLetter), readedLine.indexOf(";")));
                            }
                        } catch (Exception e) {}
                    }
        ...
        if (!invalidSectionList.isEmpty()) {
            for (int i = 0; i < invalidSectionList.size(); i++) {
                if (!(this.AbsoluteSubSectionMap.get(invalidSectionList.get(i)).getSectionType() == SectionType.Text)
                        || !(this.AbsoluteSubSectionMap.get(invalidSectionList.get(i)).getSectionType()
                                == SectionType.SectionHead))
                    log.error(
                            "Missing Section: " + invalidSectionList.get(i)
                                    + " in "
                                    + ConfigHandler.getFileFromConfigFile(this).getName());
            }
    ...
    public final List<String> getChangedSections() {
        ...
        while (section.hasNext()) {
            ...
            if (currentSection.getChangedDefaultValue()) changedSections.add(section.next());
        }
        if (changedSections.isEmpty()) return null;
        return changedSections;
    }

    /**
     * This will save the values of this configFile into an active Object so you can retrieve the Values ingame without
     * affecting the performance.
     */
    public final void readAndSaveConfigValues() {
        if (isConnected()) return;

        if (ConfigHandler.getConfigFileFromName(this.fileName) == null) return;

        if (!this.proxySide.isEffectiveSide(FMLCommonHandler.instance().getEffectiveSide())) return;
        try {
    ...
    /**
     * @param sectionName
     * @return - The value of this section, the Default value if the value can't be resolved, or null if the section
     *         does not exist.
     */
    public final Object getValue(String sectionName) {
        if (InitConfigObjectManager.getConfigByName(this.fileName) != null)
            if (this.getSubSection(sectionName) != null) return InitConfigObjectManager
                    .getField(InitConfigObjectManager.getConfigByName(this.fileName), sectionName);
            else {
    ...
    public final void setValue(String sectionName, Object newValue) {
        ...
        if (this.getSubSection(sectionName) != null) InitConfigObjectManager
                .changeFieldValue(InitConfigObjectManager.getConfigByName(this.fileName), sectionName, newValue);
    ...
    public final List<ByteBuf> writeFullConfigFileToBuff(ByteBufOutputStream stream) throws IOException {
        ...
        stream.writeUTF(
                parentDirectory.toURI().getPath().substring(
                        N2ConfigApi.getMCMainDir().toURI().getPath().length(),
                        parentDirectory.toURI().getPath().length()));
    ...
    public final boolean createFullConfigFileFromBuff(ByteBufInputStream stream, Config config) throws IOException {
        ...
        while (true) {
            ...
            if (currentString.equals(N2ConfigApiMessageHandler.getSignature(2))) return true;
            if (currentString.equals(N2ConfigApiMessageHandler.getSignature(1)))
                if (getAbsoluteSubSectionCount() != stream.readInt()) {
                    log.catching(
                            new Exception(
                                    "Found end signature while the total SectionCount isn't equal to the sended SectionCount!"));
                    return false;
                } else return true;
    ...
    public final int createValuesFromByteBuf(ByteBufInputStream stream) throws IOException {
        ...
        while (true) {
            ...
            if (currentString.equals(N2ConfigApiMessageHandler.getSignature(6))) return 0;
            if (currentString.equals(N2ConfigApiMessageHandler.getSignature(1))) return 1;
            if (currentString.equals(N2ConfigApiMessageHandler.getSignature(2))) return 2;
            section = getSubSection(currentString);
            if (section != null) section.createSectionValuesFromBuf(stream, config);
            else {
                log.catching(
                        new Exception(
                                "Couldn't find ConfigSection " + currentString + " in ConfigFile " + this.fileName));
                // return false;
            }
        }
    }

    private boolean createSection(ByteBufInputStream stream, Config config, String currentString,
            ConfigSectionCollection superSectionCollection, int subSectionIndex) throws IOException {
        ...
        if (section.getSectionType() == SectionType.SectionHead) {
            ...
            ConfigSectionCollection sectionCollection = new ConfigSectionCollection(
