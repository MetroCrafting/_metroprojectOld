...
public class InitConfigObjectManager {
    ...
    public static Config addConfigFile(String FileName) {
        if (ConfigList.containsKey(FileName)) log.error("The name " + FileName + " is already Used!");
        else {
    ...
    public static Config getConfigByName(String name) {
        if (ConfigList.containsKey(name)) return ConfigList.get(name);
        else return null;
    }

    public static boolean removeConfigFile(String FileName) {
        if (!ConfigList.containsKey(FileName)) log.error("Couldn't find Config " + FileName);
        else {
    ...
    public static Object getField(Config config, String sectionName) {
        if (config.ConfigSections.containsKey(sectionName)) return config.ConfigSections.get(sectionName);
        else return null;
    }
