 ...
 *         <br>
 *         This is a Class to handle synchronization internally. There is no need of using any method or field inside
 *         this Class.
 */
public class N2ConfigApiMessageHandler {
    ...
    public static String getSignature(int i) {
        final String[] PacketSignatures = new String[] { "2n4s7t@rD", // FirstPackage
    ...
    public void onPlayerJoin(PlayerLoggedInEvent event) {
        EventApi.defaultEventBusInstance
                .raiseEvent(ConfigApiEvents.instance.new onPreServerJoinConfigRecieve(event.player));
        ...
        EventApi.defaultEventBusInstance
                .raiseEvent(ConfigApiEvents.instance.new onPostServerJoinConfigRecieve(event.player));
    }

    @SubscribeEvent
    public void onPlayerLeave(PlayerLoggedOutEvent event) {
        EventApi.defaultEventBusInstance
                .raiseEvent(ConfigApiEvents.instance.new onPreServerLeaveConfigRemove(event.player));
        ...
        EventApi.defaultEventBusInstance
                .raiseEvent(ConfigApiEvents.instance.new onPostServerLeaveConfigRemove(event.player));
    }
    ...
    public static void syncValues(List<ConfigFile> configFileList, UUID playerID) {
        try {
            ...
            for (ConfigFile currentConfigFile : configFileList) {
                ...
                for (ByteBuf currentBuf : configBuf) packetList.add(new N2ConfigApiConfigPacket(currentBuf));
            ...
            if (FMLCommonHandler.instance().getEffectiveSide().isServer()) if (playerID != null) {
                List<EntityPlayerMP> playerList = MinecraftServer.getServer()
                        .getConfigurationManager().playerEntityList;
                for (EntityPlayerMP player : playerList) if (player.getUniqueID().equals(playerID))
    ...
    public static void syncFullConfigFile(ConfigFile configFile, UUID playerID) {
        try {
            ...
            String filePath = file.getAbsolutePath()
                    .substring(N2ConfigApi.getMCMainDir().getAbsolutePath().length(), file.getAbsolutePath().length());
            String fileID;

            if (filePath.equals(N2ConfigApi.getConfigDir().getAbsolutePath().toString())) fileID = "2N4$7Det@nT";
            ...
            for (int i = 0; i < packetsCount; i++) {
                ByteBuf bufff = (buff.copy(
                        i * maxByteBuffSize,
                        ((i + 1) * maxByteBuffSize) > buff.writerIndex() ? buff.writerIndex() - (i * maxByteBuffSize)
                                : maxByteBuffSize));
    ...
    public static void syncDeleteConfigFile(List<ConfigFile> configFileList, UUID playerID) {
        try {
            ...
            for (ConfigFile currentConfig : configFileList) {
                ...
                if (stream.buffer().capacity() > maxByteBuffSize
                        && configFileList.indexOf(currentConfig) < configFileList.size() - 3) {
    ...
    public static void processPackage(ByteBuf buf, EntityPlayerMP player) {
        try {
            ...
            switch (ID) {
                case "2n4$7Y!zSc":
                    updateConfigValues(stream, player);
                    break;
                case "2n4$7c$cq@e":
                    updateFullConfig(stream, player);
                    break;
                case "2n4$7D6Ite":
                    updateDeleteConfig(stream, player);
            }
            stream.close();
        } catch (Exception e) {
            log.catching(e);
    ...
    private static void updateConfigValues(ByteBufInputStream stream, EntityPlayerMP player) throws Exception {
        ...
        ConfigFileLoop: while (currentString != null) {
            configFile = ConfigHandler.getConfigFileFromName(currentString);

            if (configFile == null) {
                log.catching(
                        new Exception(
                                "Tried to update ConfigFile " + currentString
                                        + " which Couldn't be found!, continuing without any progress."));
                return;
            }

            if (FMLCommonHandler.instance().getEffectiveSide().isServer() && player != null
                    && !configFile.getPermission(player)) {
                log.warn(
                        "Player " + player.getDisplayName()
                                + " requested ConfigFile "
                                + configFile.getFileName()
                                + " to change with no permission!");
                continue;
            }

            EventApi.defaultEventBusInstance
                    .raiseEvent(ConfigApiEvents.instance.new OnPreConfigValuesChanged(configFile));
            ...
            EventApi.defaultEventBusInstance
                    .raiseEvent(ConfigApiEvents.instance.new OnPostConfigValuesChanged(configFile));
    ...
    private static void updateFullConfig(ByteBufInputStream stream, EntityPlayerMP player)
        ...
        try {
            ...
            if (currentString.equals(getSignature(0))) {
                ...
                if (fileID.equals("2N4$7Det@nT")) parentDirectory = N2ConfigApi.getConfigDir();
                else {
                    if ((parentDirectory = N2ConfigApi.getFileDirFromID(fileID)) != null) {
                        if (!(parentDirectory.getAbsolutePath().endsWith(stream.readUTF()))) {
                            log.catching(
                                    new Exception(
                                            "Directory with ID " + fileID
                                                    + " Doesn't share the same DirectoryPath as the File found with this same ID"));
                            return;
                        }
                ...
                if (player != null) if (!ConfigFileMap.containsKey(currentString + player.getUniqueID()))
            ...
            if (currentString.equals(getSignature(1))) {
                ...
                if (!(obj instanceof ConfigFile)) {
                    ...
                    cfg = (ConfigFile) obj;
                    if (player != null) if (!cfg.getPermission(player)) {
                        objectStream.close();
                        return;
                    }
                    ...
                    if (isNewConfig) EventApi.defaultEventBusInstance
                            .raiseEvent(ConfigApiEvents.instance.new OnPostConfigAdded(cfg));
                }
                objectStream.close();
            } else return;
            ...
            // parentDirectory = N2ConfigApi.registerCustomConfigDirectory(N2ConfigApi.getMCMainDir() + "/" +
            // stream.readUTF(), parentDirectoryID);
            // else if (!parentDirectory.toURI().getPath().endsWith(N2ConfigApi.getMCMainDir() + "/" +
            // stream.readUTF()))
            // {
            // log.catching(new Exception("The parentDirectoryID is already occupied, while the Directory doesn't point
            // to the same place!"));
            // return;
            // }
            //
            ...
            // if (configFile == null)
            // {
            // if (!Minecraft.getMinecraft().isSingleplayer() && !(MinecraftServer.getServer() != null &&
            ...
            // {
            // log.warn("Player " + player.getDisplayName() + " requested ConfigFile " + configFile.getFileName() + " to
            // change with no permission!");
            // return;
            // }
            ...
            if (ID != null) ConfigFileMap.remove(ID);
            log.catching(e);
        }
    }
    ...
    private static void updateDeleteConfig(ByteBufInputStream stream, EntityPlayerMP player) throws IOException {
        String currentString = stream.readUTF();
        if (currentString.equals(getSignature(0))) currentString = stream.readUTF();

        while (currentString != null) {
            if (currentString.equals(getSignature(1)) || currentString.equals(getSignature(2))) return;

            ConfigFile configFile = ConfigHandler.getConfigFileFromName(currentString);

            if (configFile == null) log.catching(new NullPointerException("Couldn't find ConfigFile " + currentString));
            else {
                if (player != null) if (!configFile.getPermission(player)) {
                    EventApi.defaultEventBusInstance
                            .raiseEvent(ConfigApiEvents.instance.new OnPreConfigRemoved(configFile));
                    ConfigHandler.removeConfigurationFile(currentString);
                    EventApi.defaultEventBusInstance
                            .raiseEvent(ConfigApiEvents.instance.new OnPostConfigRemoved(configFile));
                } else log.warn(
                        "Player " + player.getDisplayName()
                                + " requested to delete ConfigFile "
                                + configFile.getFileName()
                                + " with no permission!");
                else {
                    EventApi.defaultEventBusInstance
    ...
    private static boolean getPermission(EntityPlayerMP player) {
        return (MinecraftServer.getServer() != null
                && (Arrays.asList(MinecraftServer.getServer().getConfigurationManager().func_152606_n())
                        .contains(player.getGameProfile().toString()))
