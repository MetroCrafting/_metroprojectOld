...
public class ConfigParser {
    ...
    public static ConfigFile parseConfigFile(Configuration configuration, String newFileName) {
        ...
        for (String currentCategoryName : configuration.getCategoryNames()) {
            ConfigSectionCollection configSectionCollection = parseConfigCategory(
                    configuration.getCategory(currentCategoryName),
                    config,
                    true,
                    false);
    ...
    private static ConfigSectionCollection parseConfigCategory(ConfigCategory configCategory, Config config,
            boolean setExtraSpace, boolean useDefaultValueForConfigValue) {
        ConfigSectionCollection sectionCollection = new ConfigSectionCollection(
                configCategory.getName(),
                parseComment(configCategory.getComment()),
                setExtraSpace);
            ...
            switch (currentProperty.getType()) {
                case BOOLEAN:
                    ...
                    if (tempSection != null) {
                        ...
                        if (!useDefaultValueForConfigValue && config != null) InitConfigObjectManager.addField(
                                config,
                                currentProperty.getName(),
                                currentProperty.isList() ? currentProperty.getBooleanList()
                                        : currentProperty.getBoolean());
                    }
                    break;
                case INTEGER:
                    ...
                    if (tempSection != null) {
                        ...
                        if (!useDefaultValueForConfigValue && config != null) InitConfigObjectManager.addField(
                                config,
                                currentProperty.getName(),
                                currentProperty.isList() ? currentProperty.getIntList() : currentProperty.getInt());
                    }
                    break;
                case DOUBLE:
                    ...
                    if (tempSection != null) {
                        ...
                        if (!useDefaultValueForConfigValue && config != null) InitConfigObjectManager.addField(
                                config,
                                currentProperty.getName(),
                                currentProperty.isList() ? currentProperty.getDoubleList()
                                        : currentProperty.getDouble());
                    }
                    break;
                case STRING:
                    ...
                    if (tempSection != null) {
                        ...
                        if (!useDefaultValueForConfigValue && config != null) InitConfigObjectManager.addField(
                                config,
                                currentProperty.getName(),
                                currentProperty.isList() ? currentProperty.getStringList()
        ...
        for (ConfigCategory currentCategory : configCategory.getChildren()) {
            ConfigSectionCollection childConfigSectionCollection = parseConfigCategory(
                    currentCategory,
                    useDefaultValueForConfigValue);
     ...
     * @return
     */
    public static ConfigSectionCollection parseConfigCategory(ConfigCategory configCategory, boolean setExtraSpace,
            boolean useDefaultValueForConfigValue) {
     ...
     * @return
     */
    public static ConfigSectionCollection parseConfigCategory(ConfigCategory configCategory,
            boolean useDefaultValueForConfigValue) {
    ...
    public static ConfigSection parseBooleanProperty(Property booleanProperty, boolean useDefaultValueForConfigValue) {
        if (!booleanProperty.getType().equals(Type.BOOLEAN)) return null;
        ...
        if (booleanProperty.isList()) {
            ...
            configSection = new ConfigSection(
                    ...
                    parseComment(booleanProperty.comment),
                    useDefaultValueForConfigValue ? booleanProperty.getBooleanList() : defaultBooleanArray,
                    SectionType.BooleanArray,
                    true,
                    false);
        } else configSection = new ConfigSection(
                booleanProperty.getName(),
                parseComment(booleanProperty.comment),
                useDefaultValueForConfigValue ? booleanProperty.getBoolean()
                        : Boolean.parseBoolean(booleanProperty.getDefault()),
                SectionType.Boolean,
                true,
                false);
    ...
    public static ConfigSection parseIntegerProperty(Property integerProperty, boolean useDefaultValueForConfigValue) {
        if (!integerProperty.getType().equals(Type.INTEGER)) return null;
        ...
        if (integerProperty.isList()) {
            ...
            configSection = new ConfigSection(
                    ...
                    parseComment(integerProperty.comment),
                    useDefaultValueForConfigValue ? integerProperty.getIntList() : defaultIntArray,
                    SectionType.IntegerArray,
                    true,
                    false);
        } else configSection = new ConfigSection(
                integerProperty.getName(),
                parseComment(integerProperty.comment),
                useDefaultValueForConfigValue ? integerProperty.getInt()
                        : Integer.parseInt(integerProperty.getDefault()),
                SectionType.Integer,
                true,
                false);
    ...
    public static ConfigSection parseDoubleProperty(Property doubleProperty, boolean useDefaultValueForConfigValue) {
        if (!doubleProperty.getType().equals(Type.DOUBLE)) return null;
        ...
        if (doubleProperty.isList()) {
            ...
            configSection = new ConfigSection(
                    ...
                    parseComment(doubleProperty.comment),
                    useDefaultValueForConfigValue ? doubleProperty.getDoubleList() : defaultDoubleArray,
                    SectionType.DoubleArray,
                    true,
                    false);
        } else configSection = new ConfigSection(
                doubleProperty.getName(),
                parseComment(doubleProperty.comment),
                useDefaultValueForConfigValue ? doubleProperty.getDouble()
                        : Double.parseDouble(doubleProperty.getDefault()),
                SectionType.Double,
                true,
                false);
    ...
    public static ConfigSection parseStringProperty(Property stringProperty, boolean useDefaultValueForConfigValue) {
        if (!stringProperty.getType().equals(Type.STRING)) return null;

        ConfigSection configSection;

        if (stringProperty.isList()) configSection = new ConfigSection(
                stringProperty.getName(),
                parseComment(stringProperty.comment),
                useDefaultValueForConfigValue ? stringProperty.getStringList() : stringProperty.getDefaults(),
                SectionType.StringArray,
                true,
                false);
        else configSection = new ConfigSection(
                stringProperty.getName(),
                parseComment(stringProperty.comment),
                useDefaultValueForConfigValue ? stringProperty.getString() : stringProperty.getDefault(),
                SectionType.String,
                true,
                false);
    ...
    public static ConfigSection parseProperty(Property property, boolean useDefaultValueForConfigValue) {
        ...
        if (property.isList()) configSection = new ConfigSection(
                property.getName(),
                parseComment(property.comment),
                useDefaultValueForConfigValue ? property.getStringList() : property.getDefaults(),
                SectionType.StringArray,
                true,
                false);
        else configSection = new ConfigSection(
                property.getName(),
                parseComment(property.comment),
                useDefaultValueForConfigValue ? property.getString() : property.getDefault(),
                SectionType.String,
                true,
                false);
    ...
    private static String[] parseComment(String comment) {
        if (comment != null) if (comment.contains("\n")) return comment.split("\n");
