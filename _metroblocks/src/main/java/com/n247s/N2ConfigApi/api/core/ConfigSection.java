...
public class ConfigSection implements Serializable {
     ...
     * @param separateLines - If the sectionType is an ArrayType, this will cause each defaultValue to be printed on a
     *                      separate line.
     ...
     * @param defaultValue - This will be printed if the ConfigurationFile is created/regenerated.
     * @param sectionType  - This will define which type of data you wan't to add.
     * @param hideOutLines - If the sectionType is an StringArray, this will cause each String to be printed on a
     *                     separate line.
     */
    public ConfigSection(String sectionName, String[] description, Object defaultValue, SectionType sectionType,
    ...
    public final String getHeadEnder() {
        if (this.SectionHeadEnder != null) if (this.SectionHeadEnder.length() == 0) return this.sectionStarter;
    ...
    protected final void writeFullSectionToByteBuf(ByteBufOutputStream stream, Object value) throws IOException {
        ...
        for (String currentLine : this.description) stream.writeUTF(currentLine);
        ...
        if (this.sectionType.isArray) {
            if (this.sectionType == SectionType.SectionHead) stream.writeInt((int) value);
            else {
                List valueList = Arrays.asList((Object[]) value);
                stream.writeInt(valueList.size());
                for (Object currentValue : valueList) writeValueToByteBuf(stream, currentValue);
            }
    ...
    protected final void WriteSectionValuesToByteBuf(ByteBufOutputStream stream, Object value) throws IOException {
        if (this.sectionType.equals(SectionType.Text) || this.sectionType.equals(SectionType.SectionHead)) return;
    ...
    protected final void createFullSectionFromBuf(ByteBufInputStream stream, Config config) throws IOException {
        ...
        for (int i = 0; i < descriptionLength; i++) description[i] = stream.readUTF();
        this.description = description;
        ...
        if (this.sectionType.isArray) {
            ...
            if (this.sectionType != SectionType.SectionHead) {
                for (int i = 0; i < arrayLength; i++) {
                    if (this.sectionType != SectionType.Text) array[i] = readValueFromByteBuf(stream);
                }
        ...
        if (config != null) InitConfigObjectManager.addField(config, this.sectionName, this.defaultValue);
    }
    ...
    protected final void createSectionValuesFromBuf(ByteBufInputStream stream, Config config) throws IOException {
        if (this.sectionType != SectionType.values()[stream.readInt()]) {
            log.catching(
                    new Exception(
                            "ConfigSection " + this.sectionName + " has a discrepancy from the recieved information!"));
            return;
        }
