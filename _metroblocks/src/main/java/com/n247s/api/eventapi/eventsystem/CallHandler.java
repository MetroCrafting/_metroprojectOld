...
public abstract class CallHandler {
    ...
    public void removeListener(CustomEventSubscribe.Priority priority, Object Listener) {
        ...
        if (currenInstanceMap == null) log.catching(
                new NullPointerException(
                        "There is no Listner registered for this priority level " + priority.toString() + "!"));

        if (currenInstanceMap.containsKey(Listener)) currenInstanceMap.remove(Listener);
        else log.catching(
                new NullPointerException(
                        "Current Listner has either changed its annotation value's or is not registered!"));
    }
}
