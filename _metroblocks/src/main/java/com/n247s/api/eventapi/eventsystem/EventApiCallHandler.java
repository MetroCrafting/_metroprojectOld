...
public class EventApiCallHandler extends CallHandler {
    ...
    /**
     * @param orderList - List with Classes and instances that should be called in a the specific given
     *                  order(ascending). Note that all Classes/instances that are not included in the list will be
     *                  called afterwards in order of registering. (In other words, giving an empty list(null) will
     *                  cause all Classes/instances being called in registering order)
     * @return
     * @throws IllegalArgumentException - If a CustomEventSubscribed Method contains more than one parameter, or if the
     *                                  parameter is not an instance of EventType.class.
     */
        ...
        if (orderList != null) {
            try {
                ...
                for (int i = 0; i < orderList.size(); i++) {
                    ...
                    if (!this.entryCheckList.contains(originalEntry)) continue;

                    Class clazz;
                    if (!(originalEntry instanceof Class)) clazz = originalEntry.getClass();
                    ...
                    if (containsMethod) containsMethod = false;
                    else log.catching(
                            new IllegalArgumentException(
                                    "Class " + clazz.getName() + " doesn't contain an eventMethod!"));
                }
     ...
     *                  Note that all Classes/instances that are not included in the list will be called afterwards in
     *                  order of registering. (In other words, giving an empty list(null) will cause all
     *                  Classes/instances being called in registering order)
    ...
    protected boolean CallInstancesInOrderWithBlackList(EventType eventTypeInstance, List orderList, List blackList) {
        ...
        for (int i = 0; i < blackList.size(); i++) {
            ...
            if (orderList.contains(current)) orderList.remove(current);
        }
        ...
        if (blackList.size() > 0) calledInstanceList.addAll(blackList);
        ...
        try {
            ...
            for (int i = 0; i < whiteList.size(); i++) {
                ...
                if (containsMethod) containsMethod = false;
                        ...
                        new IllegalArgumentException("Class " + clazz.getName() + " doesn't contain an eventMethod!"));
            }
        } catch (Exception e) {
            log.catching(e);
        }
