...
public class RenderHelper3D {
    ...
    public static void renderBlock(double x, double y, double z, double width, double height, double length,
            double rotateX, double rotateY, double rotateZ, double red, double green, double blue, double alpha) {
        GL11.glPushMatrix();
    ...
    public static void renderBlock(Block block, double x, double y, double z, double width, double height,
            double length, double rotateX, double rotateY, double rotateZ) {
        renderBlock(block, x, y, z, width, height, length, rotateX, rotateY, rotateZ, ForgeDirection.EAST);
    }

    public static void renderBlock(Block block, double x, double y, double z, double width, double height,
            double length, double rotateX, double rotateY, double rotateZ, ForgeDirection direction) {
        GL11.glPushMatrix();
        GL11.glTranslated(x, y, z);
        ...
        for (int blockX = 0; blockX < blocksX; blockX++) {
            for (int blockY = 0; blockY < blocksY; blockY++) {
                for (int blockZ = 0; blockZ < blocksZ; blockZ++) {
                    ...
                    double boundX = 1;
                    if (blockX == blocksX - 1) boundX = sizeX - blockX;
                    double boundY = 1;
                    if (blockY == blocksY - 1) boundY = sizeY - blockY;
                    double boundZ = 1;
                    if (blockZ == blocksZ - 1) boundZ = sizeZ - blockZ;
    ...
    public static void renderItem(ItemStack stack, double x, double y, double z, double rotationX, double rotationY,
            double rotationZ, double size, ForgeDirection direction, double moveX, double moveY, double moveZ) {
    ...
    public static void renderIcon(IIcon p_77020_2_, int p_77020_3_, float p_77020_5_, float p_77020_6_,
            float p_77020_7_, int pass, boolean hasEffect, int stackSize, boolean isBlock) {
        Tessellator tessellator = Tessellator.instance;
        int spritenumber = 0;
        if (!isBlock) spritenumber = 1;
    ...
    public static void renderDroppedItem(ItemStack stack, IIcon p_77020_2_, int p_77020_3_, float p_77020_5_,
            float p_77020_6_, float p_77020_7_, int pass) {
        renderIcon(
                p_77020_2_,
                p_77020_3_,
                p_77020_5_,
                ...
                p_77020_7_,
                pass,
                stack.hasEffect(pass),
                stack.stackSize,
                stack.getItemSpriteNumber() == 0);
    }

    public static void renderLightning(double x, double y, double z, Vec3 start, Vec3 end, double thickness,
            int amountOfHalf, int maxDifference) {
        ...
        for (int i = 1; i <= amountOfHalf; i++) {
            ...
            for (int j = 1; j <= numberOfPoints; j++) {
                if (j / 2 != j / 2D) {
                    ...
                    points[index] = points[preIndex]
                            .addVector(points[postIndex].xCoord, points[postIndex].yCoord, points[postIndex].zCoord);
                    ...
                    Vec3 sideVec = Vec3.createVectorHelper(
    ...
    public static void renderWorldCubes(RenderBlocks renderer, Block block, int meta, ArrayList<CubeObject> cubes,
            int x, int y, int z) {
        for (int i = 0; i < cubes.size(); i++) {
            ...
            if (cubes.get(i).block != null) if (cubes.get(i).meta != -1) {
                ...
                RenderHelper3D.renderBlocks.setRenderBounds(
                        cubes.get(i).minX,
                        cubes.get(i).minY,
                        cubes.get(i).minZ,
                        cubes.get(i).maxX,
