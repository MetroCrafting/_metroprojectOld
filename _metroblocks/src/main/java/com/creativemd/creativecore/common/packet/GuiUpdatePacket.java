...
public class GuiUpdatePacket extends CreativeCorePacket {
    ...
    public void executeClient(EntityPlayer player) {
        if (player.openContainer instanceof ContainerSub)
            if (isOpening) ((ContainerSub) player.openContainer).gui.getLayers().get(layer).readFromOpeningNBT(value);
            else((ContainerSub) player.openContainer).gui.getLayers().get(layer).readFromNBT(value);
    }
