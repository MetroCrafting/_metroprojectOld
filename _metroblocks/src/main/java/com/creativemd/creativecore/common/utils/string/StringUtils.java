...
public class StringUtils {
    ...
    public static Object[] StringToObjects(String input) {
        if (input.startsWith("{") && input.endsWith("}")) // String example: {I:1;S:jfkjfwiuej3keksa;NBT:{S:SJDkjskajd}}
        {
            ...
            while (index < end) {
                ...
                if (currentChar.equals("{")) index += getBraceLength(index, input) - 1;
                if (currentChar.equals(";")) breaks.add(index);
                index++;
            }
            ArrayList objects = new ArrayList();
            for (int i = 0; i < breaks.size(); i++) {
                int beforeIndex = 1;
                if (i > 0) beforeIndex = breaks.get(i - 1) + 1;
    ...
    protected static int getBraceLength(int StartIndex, String input) {
        ...
        if (input.substring(StartIndex, StartIndex + 1).equals("{")) braces++;
        index++;
        while (index < input.length() && braces > 0) {
            if (input.substring(index, index + 1).equals("{")) braces++;
            if (input.substring(index, index + 1).equals("}")) braces--;
            index++;
        }
