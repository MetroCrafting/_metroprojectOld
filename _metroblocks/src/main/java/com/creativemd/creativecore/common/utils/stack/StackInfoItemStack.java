...
public class StackInfoItemStack extends StackInfo implements IStackLoader {
    ...
    public StackInfo getStackInfo(Object item) {
        if (item instanceof ItemStack) {
            if (((ItemStack) item).getItemDamage() == OreDictionary.WILDCARD_VALUE) {
                ...
                else return new StackInfoItem(((ItemStack) item).getItem());
            }
    ...
    public StackInfo getStackInfoFromString(String input) {
        ...
        if (objects.length == 2 && objects[0] instanceof Item && objects[1] instanceof Integer) {
            ...
            StackInfoItemStack result = new StackInfoItemStack(
                    new ItemStack((Item) objects[0], 1, (Integer) objects[1]));
            result.needNBT = true;
    ...
    public String toString() {
        if (needNBT) {
            ...
            else return StringUtils.ObjectsToString(stack.getItem(), stack.getItemDamage(), stack.stackTagCompound);
        } else {
    ...
    protected boolean isStackInstanceIgnoreSize(ItemStack stack) {
        if (stack.getItem() != this.stack.getItem()) return false;
        if (stack.getItemDamage() != this.stack.getItemDamage()) return false;
        if (needNBT) {
            if (stack.stackTagCompound == this.stack.stackTagCompound) return true;
    ...
    public boolean isInstanceIgnoreSize(StackInfo info) {
        if (info instanceof StackInfoItemStack) {
            if (((StackInfoItemStack) info).stack.getItem() != stack.getItem()) return false;
            if (((StackInfoItemStack) info).stack.getItemDamage() != stack.getItemDamage()) return false;
    ...
    public ItemStack getItemStack(int stacksize) {
        ...
        if (needNBT) stack.stackTagCompound = (NBTTagCompound) this.stack.stackTagCompound.copy();
        return stack;
    }
