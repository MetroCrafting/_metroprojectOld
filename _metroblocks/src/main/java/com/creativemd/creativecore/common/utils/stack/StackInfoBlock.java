...
public class StackInfoBlock extends StackInfo implements IStackLoader {
    ...
    public StackInfo getStackInfo(Object item) {
        Block block = null;
        if (item instanceof Block) block = (Block) item;
        if (item instanceof Item) block = Block.getBlockFromItem((Item) item);
        if (block != null) return new StackInfoBlock(block);
        return null;
    }
    ...
    public StackInfo getStackInfoFromString(String input) {
        ...
        if (block != null && !(block instanceof BlockAir)) return new StackInfoBlock(block);
        return null;
    }
    ...
    public boolean isInstanceIgnoreSize(StackInfo info) {
        if (info instanceof StackInfoBlock) return ((StackInfoBlock) info).block == block;
