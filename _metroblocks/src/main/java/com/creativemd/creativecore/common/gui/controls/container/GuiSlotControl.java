...
public class GuiSlotControl extends GuiControl {
    ...
    public void drawControl(FontRenderer renderer) {
        ...
        if (newStack != null) {
            if (stack != null) newStack.stackSize += stack.stackSize;
            stack = newStack;
        }

        if (slot.slot instanceof SlotInput) {
            ItemStack backgroundStack = null;
            if (((SlotInput) slot.slot).input != null) backgroundStack = ((SlotInput) slot.slot).input.getItemStack(1);
    ...
    public void drawItemStack(ItemStack stack, FontRenderer renderer) {
        try {
            ...
            if (slot.slot.getStack() != null) font = slot.slot.getStack().getItem().getFontRenderer(stack);
    ...
    public boolean mousePressed(int posX, int posY, int button) {
        ...
        if (stack != null && button < 2) {
            ...
            if (slot.slot.getHasStack()) stackSize -= slot.slot.getStack().stackSize;
            if (!slot.slot.isItemValid(dragged)) stackSize = 0;
    ...
    public boolean mouseDragged(int posX, int posY, int button, long time) {
        if (startSlot == null && !isDragged()) {
            for (int i = 0; i < parent.controls.size(); i++) {
                if (parent.controls.get(i) instanceof GuiSlotControl
                        && ((GuiSlotControl) parent.controls.get(i)).isDragged()) {
                    ...
                    if (stackSize > 0 && canStack) {
                        ...
                        for (int j = 0; j < startSlot.stackSize.length; j++) newStackSize[j] = startSlot.stackSize[j];
    ...
    /*
     * @Override public void mouseMove(int posX, int posY, int button){ }
     */
        ...
        if (isDragged() && stackSize.length > 1) {
            ...
            for (int i = 0; i < parent.controls.size(); i++) {
                if (parent.controls.get(i) instanceof GuiSlotControl
                    ...
                    slotArray[((GuiSlotControl) parent.controls.get(i)).index] = ((GuiSlotControl) parent.controls
                            .get(i)).slot.getID();
                    ((GuiSlotControl) parent.controls.get(i)).startSlot = null;
    ...
    public ItemStack getItemStackByIndex(int index) {
        if (index >= dragged.stackSize) return null;
        ItemStack result = dragged.copy();
        if (isRightClick) result.stackSize = 1;
        else {
            int used = 0;
            int StackPerSlot = MathHelper.floor_float((float) result.stackSize / (float) stackSize.length);
            for (int i = 0; i < index; i++) used += Math.min(StackPerSlot, stackSize[i]);
            if (used < result.stackSize) result.stackSize = Math.min(StackPerSlot, stackSize[index]);
        }
        return result;
    }
    ...
    public void updateHandStack() {
        ...
        for (int i = 0; i < stackSize.length; i++) used += Math.min(StackPerSlot, stackSize[i]);
