...
public class StackInfoMaterial extends StackInfo implements IStackLoader {
    ...
    public StackInfo getStackInfoFromString(String input) {
        ...
        if (block != null) return new StackInfoMaterial(block.getMaterial());
        return null;
    }
    ...
    public String toString() {
        for (Object name : Block.blockRegistry.getKeys()) {
            ...
            if (block != null && block.getMaterial() == material) return (String) name;
        }
        return null;
    }
    ...
    public boolean isInstanceIgnoreSize(StackInfo info) {
        if (info instanceof StackInfoMaterial) return ((StackInfoMaterial) info).material == material;
        if (info instanceof StackInfoBlock) return ((StackInfoBlock) info).block.getMaterial() == material;
        if (info instanceof StackInfoItemStack) {
            Block block = Block.getBlockFromItem(((StackInfoItemStack) info).stack.getItem());
            if (block != null) return block.getMaterial() == material;
        }
        return false;
    }
