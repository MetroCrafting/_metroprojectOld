...
public class GuiItemStackSelector extends GuiComboBoxExtension {
    ...
    public GuiItemStackSelector(String name, EntityPlayer player, int x, int y, int width, int height,
            GuiComboBox comboBox, boolean onlyBlocks, String search) {
        ...
        for (int i = 0; i < player.inventory.mainInventory.length; i++) {
            if (player.inventory.mainInventory[i] != null) inv.add(player.inventory.mainInventory[i]);
        }
    ...
    public static String getItemName(ItemStack stack) {
        ...
        try {
            ...
            else itemName = Item.itemRegistry.getNameForObject(stack.getItem());
        }
        return itemName;
    }

    public static boolean shouldShowItem(boolean onlyBlocks, String search, ItemStack stack) {
        if (onlyBlocks && Block.getBlockFromItem(stack.getItem()) instanceof BlockAir) return false;
    ...
    public void refreshControls() {
        if (stacks != null) {
            gui.controls.clear();
            /*
             * for (int i = 0; i < lines.size(); i++) { int color = 14737632; if(i == selected) color = 16777000;
             * GuiLabelClickable label = new GuiAvatarLabel(lines.get(i), 3, 1+i*22, color, new
             * AvatarItemStack(stacks.get(i))); label.width = width-20; label.height = 22; addControl(label); }
             */
            int height = 0;
            ...
            for (int i = 0; i < inv.size(); i++) {
                if (shouldShowItem(onlyBlocks, search, inv.get(i))) {
                    ...
                    addControl(
                            new SlotControlNoSync(
                                    new SlotPreview(basic, 0, (count - row * SlotsPerRow) * 18, height + row * 18)));
                    count++;
                }
            }
