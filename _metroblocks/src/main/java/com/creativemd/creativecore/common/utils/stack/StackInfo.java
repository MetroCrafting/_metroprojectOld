...
public abstract class StackInfo {
    ...
    public static StackInfo parseObject(Object stack) {
        ...
        if (stack instanceof StackInfo) return (StackInfo) stack;
    ...
    public int indexOf(ArrayList<StackInfo> infos) {
        for (int i = 0; i < infos.size(); i++) {
            if (infos.get(i).equalsIgnoreSize(this)) return i;
        }
        return -1;
    }
    ...
    public boolean equals(Object object) {
        return object instanceof StackInfo && ((StackInfo) object).stackSize == this.stackSize
                && equalsIgnoreSize(object);
    }
    ...
    public boolean isInstance(StackInfo info) {
        if (isInstanceIgnoreSize(info)) {
            if (stackSize < info.stackSize) return false;
        }
        return false;
    }
