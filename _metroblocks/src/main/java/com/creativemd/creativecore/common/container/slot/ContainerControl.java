...
public abstract class ContainerControl {
    ...
    /**
     * If two players are looking into the same inventory (TileEntity). NOTE: Does only work for tileEntities because it
     * uses the coordinates to identify a container
     */
    ...
    public void sendNBTUpdate(int type, NBTTagCompound nbt) {
        ...
        else PacketHandler.sendPacketToPlayer(
                new ContainerControlUpdatePacket(parent.getLayerID(), id, nbt),
                (EntityPlayerMP) parent.player);
    }
    ...
    /*
     * public void onControlChanged() { boolean shouldSync = sync == SyncControl.BOTH;
     * if(FMLCommonHandler.instance().getEffectiveSide().isClient() && sync == SyncControl.CLIENTTOSERVER) shouldSync =
     * true; if(FMLCommonHandler.instance().getEffectiveSide().isServer() && sync == SyncControl.SERVERTOCLIENT)
     * shouldSync = true; if(shouldSync) { } }
     */
    ...
    /*
     * public boolean doesControlSupportSync() { return containsClass(this.getClass()); }
     */
    ...
    public static void registerControl(String id, Class<? extends ContainerControl> ControlClass) {
        if (containsID(id)) throw new IllegalArgumentException("ID=" + id + " is already taken!");
