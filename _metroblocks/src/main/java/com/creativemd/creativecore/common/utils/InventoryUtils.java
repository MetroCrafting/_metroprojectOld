...
public class InventoryUtils {
    ...
    public static NBTTagCompound saveInventory(ItemStack[] inventory) {
        ...
        for (int i = 0; i < inventory.length; i++) {
            ...
            if (inventory[i] != null) inventory[i].writeToNBT(newNBT);
    ...
    public static InventoryBasic loadInventoryBasic(NBTTagCompound nbt, int length) {
        ...
        if (nbt != null) {
            ...
            for (int i = 0; i < stacks.length; i++) {
                if (length > i) basic.setInventorySlotContents(i, stacks[i]);
            }
        }
        return basic;
    }
    ...
    public static boolean isItemStackEqual(ItemStack stack, ItemStack stack2) {
        if (stack != null && stack2 != null
            ...
            if (!stack.hasTagCompound() && !stack2.hasTagCompound()) return true;
            else if (stack.hasTagCompound() != stack2.hasTagCompound()) return false;
            else if (stack.stackTagCompound.equals(stack2.stackTagCompound)) return true;
            ...
            for (int i = 0; i < inventory.getSizeInventory(); i++) {
                if (isItemStackEqual(inventory.getStackInSlot(i), stack)) {
                    ...
                    if (amount > 0) {
                        ...
                        if (inventory.getStackInSlot(i).stackSize <= 0) inventory.setInventorySlotContents(i, null);
    ...
    public static void cleanInventory(IInventory inventory) {
        for (int i = 0; i < inventory.getSizeInventory(); i++) {
            ...
            if (stack != null && stack.stackSize == 0) inventory.setInventorySlotContents(i, null);
        }
    }
    ...
    public static int consumeStackInfo(StackInfo info, IInventory inventory, ArrayList<ItemStack> consumed) {
        ...
        if (consumed != null) consumed.addAll(stacks);
        return stackSize;
    }
