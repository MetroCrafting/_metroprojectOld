...
public class HashMapList<K, V> {
    ...
    public K getLast() {
        if (keys.size() > 0) return keys.get(keys.size() - 1);
        return null;
    }
    ...
    public K getKey(V value) {
        for (int i = 0; i < values.size(); i++) {
            ...
            if (index != -1) return getKey(index);
        }
        return null;
    }
    ...
    public void add(K key, ArrayList<V> values) {
        int index = indexOfKey(key);
        if (index != -1) this.values.get(index).addAll(values);
        else {
            keys.add(key);
    ...
    public void add(K key, V value) {
        int index = indexOfKey(key);
        if (index != -1) values.get(index).add(value);
        else {
            keys.add(key);
            ArrayList<V> newList = new ArrayList<V>();
            newList.add(value);
    ...
    public boolean removeValue(K key, V value) {
        ...
        if (values != null) return values.remove(value);
        return false;
    }

    public boolean removeValue(V value) {
        for (int i = 0; i < values.size(); i++) {
            if (values.get(i).remove(value)) return true;
        }
        return false;
    }
