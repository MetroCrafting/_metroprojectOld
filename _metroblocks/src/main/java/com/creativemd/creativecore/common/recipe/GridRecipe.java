...
public class GridRecipe extends Recipe {
    ...
    protected ItemStack[] getObjectInValidOrder(IInventory inventory, int InvWidth, int InvHeigt) {
        if (InvWidth < this.width || InvHeigt < this.height) return null;
        ...
        if (gridWidth != this.width || gridHeight != this.height) return null;
    ...
    protected boolean isStackValid(ItemStack stack, StackInfo input) {
        if (stack == null && input == null) return true;
        if (input != null) return input.isInstance(stack);
        return false;
    }
    ...
    protected boolean isValidRecipe(ItemStack[] inv) {
        // Check if it's valid
        for (int i = 0; i < input.length; i++) {
            if (!isStackValid(inv[i], input[i])) return false;
        }
        return true;
    }
}
