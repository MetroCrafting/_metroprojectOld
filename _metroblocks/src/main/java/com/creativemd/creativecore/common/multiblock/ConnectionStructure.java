    ...
    public static TileEntity getLowestPoint(ArrayList<TileEntity> blocks) {
        ...
        for (int i = 0; i < blocks.size(); i++) {
            ...
            if (lowest == null || lowestPos > pos) {
                ...
                if (blocks.get(i).xCoord > lowest.xCoord) isLowest = true;
                else if (blocks.get(i).xCoord == lowest.xCoord)
                    if (blocks.get(i).yCoord > lowest.yCoord) isLowest = true;
                else if (blocks.get(i).yCoord == lowest.yCoord)
                    if (blocks.get(i).zCoord > lowest.zCoord) isLowest = true;
                if (isLowest) {
                    lowest = blocks.get(i);
                    lowestPos = pos;
                }
