...
public class StackInfoOre extends StackInfo implements IStackLoader {
    ...
    public StackInfo getStackInfo(Object item) {
        if (item instanceof String) return new StackInfoOre((String) item);
        if (item instanceof ArrayList) {
            ...
            for (int i = 0; i < ores.size(); i++) {
                ...
                for (int j = 0; j < oreIDsofStack.length; j++)
                    if (i == 0 || oresIDs.contains(oreIDsofStack[j])) neworesIDs.add(oreIDsofStack[j]);
                oresIDs = neworesIDs;
            }
            if (oresIDs.size() == 1) return new StackInfoOre(OreDictionary.getOreName(oresIDs.get(0)));
        }
        return null;
    }
    ...
    protected boolean isStackInstanceIgnoreSize(ItemStack stack) {
        ...
        for (int i = 0; i < ores.length; i++) if (OreDictionary.getOreName(ores[i]).equals(ore)) return true;
        return false;
    }

    @Override
    public boolean isInstanceIgnoreSize(StackInfo info) {
        if (info instanceof StackInfoOre) return ((StackInfoOre) info).ore.equals(ore);
        return false;
    }
