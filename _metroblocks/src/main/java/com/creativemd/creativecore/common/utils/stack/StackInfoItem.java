...
public class StackInfoItem extends StackInfo implements IStackLoader {
    ...
    public StackInfo getStackInfo(Object item) {
        if (item instanceof Item && !(item instanceof ItemBlock)) return new StackInfoItem((Item) item);
        return null;
    }
    ...
    public StackInfo getStackInfoFromString(String input) {
        ...
        if (item != null) return new StackInfoItem(item);
        return null;
    }
    ...
    public boolean isInstanceIgnoreSize(StackInfo info) {
        if (info instanceof StackInfoItem) return ((StackInfoItem) info).item == item;
        if (info instanceof StackInfoItemStack) return item == ((StackInfoItemStack) info).stack.getItem();
        return false;
    }
