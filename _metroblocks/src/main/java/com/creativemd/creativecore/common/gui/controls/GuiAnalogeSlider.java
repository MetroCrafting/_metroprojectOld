...
public class GuiAnalogeSlider extends GuiControl {
    ...
    public GuiAnalogeSlider(String name, int x, int y, int width, int height, int rotation, float value, float minValue,
            float maxValue) {
    ...
    public GuiAnalogeSlider(String name, int x, int y, int width, int height, float value, float minValue,
            float maxValue) {
    ...
    public void drawControl(FontRenderer renderer) {
        ...
        if (isMouseOver((int) mouse.x, (int) mouse.y)) mouseEvent = 2;

        if (!this.enabled) mouseEvent = 0;

        GL11.glEnable(GL11.GL_BLEND);
        OpenGlHelper.glBlendFunc(770, 771, 1, 0);
        GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
        /*
         * parameters - this.posX - this.posY - this.width - this.height - this.value - this.maxValue - this.minValue
         */
        ...
        renderer.drawStringWithShadow(
                text,
                width / 2 - renderer.getStringWidth(text) / 2,
                (this.height - 8) / 2,
                White);

        /*
         * GL11.glPushMatrix(); mc.getTextureManager().bindTexture(buttonTextures);
         * RenderHelper2D.drawTexturedModalRect(0, this.posX + this.width, this.posY, 0, 0, this.width, this.height);
         * GL11.glPopMatrix(); GL11.glPushMatrix(); mc.getTextureManager().bindTexture(buttonTextures);
         * RenderHelper2D.drawTexturedModalRect(0, (int)((float)this.posX + ((float)this.width * (((this.value -
         * this.minValue) / (this.maxValue - this.minValue))))), this.posY, 0, 0, (int)((float)this.width * (5F/85F)) ,
         * this.height); GL11.glPopMatrix();
         */
    ...
    public void mouseMove(int posX, int posY, int button) {
        Vector2d mouse = parent.getMousePos();
        if (grabbedSlider) {
            if (mouse.x < this.posX) this.value = this.minValue;
            else if (mouse.x > this.posX + this.width) this.value = this.maxValue;
            else this.value = this.minValue
                    + (float) ((this.maxValue - this.minValue) * ((float) (mouse.x - this.posX) / (float) this.width));
            setValue(value);
            // return true;
        }
        // return false;
    ...
    public void mouseReleased(int posX, int posY, int button) {
        if (this.grabbedSlider) grabbedSlider = false;
    }
}
