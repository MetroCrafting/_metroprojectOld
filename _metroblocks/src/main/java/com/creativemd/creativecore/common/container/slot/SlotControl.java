...
public class SlotControl extends ContainerControl {
    ...
    public void detectChange() {
        if (!ItemStack.areItemStacksEqual(lastSended, slot.getStack())) // || (lastSended != null && slot.getHasStack()
    ...
    public void writeToNBT(NBTTagCompound nbt) {
        if (slot.getHasStack()) slot.getStack().writeToNBT(nbt);
        if (slot.getHasStack()) lastSended = slot.getStack().copy();
        else lastSended = null;
        NBTTagCompound dragSlot = new NBTTagCompound();
        if (parent.player.inventory.getItemStack() != null) parent.player.inventory.getItemStack().writeToNBT(dragSlot);
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        switch (nbt.getInteger("type")) {
            ...
            case 0:
                ...
                if (nbt.hasKey("drag")) dragStack = ItemStack.loadItemStackFromNBT(nbt.getCompoundTag("drag"));
    ...
    public void dropItem(boolean ctrl) {
        if (slot.getHasStack()) {
            ...
            if (ctrl) {
                ...
                if (newStack.stackSize == 0) slot.putStack(null);
                else slot.putStack(newStack);
            }
    ...
    public static ItemStack putItemInSlot(Slot slot, ItemStack stack) {
        ...
        if (canStack && slot.isItemValid(stack)) {
            ...
            if (slot.getHasStack()) stackSize -= slot.getStack().stackSize;
            ...
            if (stackSize > 0) {
                ...
                if (slot.getHasStack()) inSlot.stackSize += slot.getStack().stackSize;
    ...
    public void switchItems() {
        ...
        if (slot.isItemValid(hand)) {
            ...
            if (canStack && (!slot.getHasStack() || slot.canTakeStack(parent.player))) {
                ...
                if (slot.getHasStack()) stackSize -= slotItem.stackSize;

                stackSize = Math.min(stackSize, neededStackSize);
                ItemStack inSlot = hand.copy();

                inSlot.stackSize = stackSize;
                if (slot.getHasStack()) inSlot.stackSize += slotItem.stackSize;

                slot.putStack(inSlot);
                int left = neededStackSize - stackSize;
                if (left > 0) hand.stackSize = left;
                else inventoryplayer.setItemStack(null);
            } else {
                int left = hand.stackSize - stackSize;
                if (left > 1) parent.player.dropPlayerItemWithRandomChoice(slotItem.copy(), true);
                ItemStack inSlot = hand.copy();
                inSlot.stackSize = Math.min(stackSize, hand.stackSize);
                slot.putStack(inSlot);
                hand.stackSize -= Math.min(stackSize, hand.stackSize);
                if (hand.stackSize == 0) inventoryplayer.setItemStack(slotItem.copy());
            }
        }
    }
                        ...
                        if (parent.controls.get(i) instanceof SlotControl) {
                            ...
                            if (mergeSlot.inventory != slot.inventory) {
                                ...
                                if (canStack && mergeSlot.getHasStack() && mergeSlot.getStack().stackSize > 0) {
                                    ...
                                    if (mergeSlot.getStack().stackSize == 0) mergeSlot.putStack(null);
                                    return;
                                }
                            }
                        }
                ...
                for (int i = 0; i < parent.controls.size(); i++) {
                    if (parent.controls.get(i) instanceof SlotControl) {
                        ...
                        if (mergeSlot.inventory != slot.inventory) {
                            ...
                            if (copy != null) stack.stackSize -= amount - copy.stackSize;
    ...
    public void onSlotClicked(int mouseButton, boolean shift, int scrolled) {
        ...
        if (mouseButton == 0) {
            ...
            if (hand != null) {
                if (slot.isItemValid(hand)) {
                    ...
                    if (canStack) {
                        ...
                        if ((slot.getHasStack() && stacksize > slotItem.stackSize)
                                || (!slot.getHasStack() && stacksize > 0)) {
                            ItemStack inSlot = hand.copy();
                            inSlot.stackSize = stacksize;
                            slot.putStack(inSlot);
                            hand.stackSize--;
                            if (hand.stackSize == 0) inventoryplayer.setItemStack(null);
                        }
                    } else {
                        switchItems();
                    }
            ...
            if (slot.canTakeStack(parent.player)) transferIntoOtherInventory(scrolled == -1, 1);
        }
    }
    ...
    public void splitStack(int[] slots, ItemStack stack, boolean isRightClick) {
        ...
        for (int i = 0; i < slots.length; i++) {
            ...
            if (use > 0) {
                int size = use;
                if (control.slot.getHasStack()) size += control.slot.getStack().stackSize;
        ...
        if (stack.stackSize <= 0) parent.player.inventory.setItemStack(null);
        else parent.player.inventory.setItemStack(stack.copy());
    }
