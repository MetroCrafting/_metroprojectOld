    ...
    public void onClicked(ControlClickEvent event) {
        if (event.source.is("run")) {
            ...
            if (!any && stackFilter != null) {
                nbt.setString(
                        "filterBlock",
                        Block.blockRegistry.getNameForObject(Block.getBlockFromItem(stackFilter.getItem())));
                if (meta) nbt.setInteger("filterMeta", stackFilter.getItemDamage());
            }
            ...
            if (replace) {
                ...
                if (stackReplace != null) {
                    ...
                    if (metaR) nbt.setInteger("replaceMeta", stackReplace.getItemDamage());
                }
            }
            ...
            if (!replace && !colorize) openButtonDialogDialog("You have to select a task!", "ok");
            else sendPacketToServer(0, nbt);
        }
    }
