...
public class LittleTileTileEntity extends LittleTileBlock {
    ...
    public void updatePacket(NBTTagCompound nbt) {
        ...
        if (!firstSended) {
            ...
            if (packet instanceof S35PacketUpdateTileEntity) {
                ...
                if (newNBT != null) nbt.setTag("tileentity", newNBT);
            } else {
    ...
    public void receivePacket(NBTTagCompound nbt, NetworkManager net) {
        ...
        if (nbt.getBoolean("isFirst")) {
            ...
            if (tileEntity != null) tileEntity.onDataPacket(
                    net,
    ...
    public void renderTick(double x, double y, double z, float partialTickTime) {
        if (tileEntity != null) {
            ...
            if (te.getDistanceFrom(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ)
    ...
    public double getMaxRenderDistanceSquared() {
        if (tileEntity != null) return tileEntity.getMaxRenderDistanceSquared();
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        if (tileEntity != null) return tileEntity.getRenderBoundingBox()
                .getOffsetBoundingBox(cornerVec.getPosX(), cornerVec.getPosY(), cornerVec.getPosZ());
