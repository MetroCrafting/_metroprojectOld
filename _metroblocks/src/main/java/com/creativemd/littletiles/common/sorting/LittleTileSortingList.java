...
public class LittleTileSortingList {
    ...
    /*
     * public static LittleTile getLittleTile(ItemStack stack) { if(stack == null || stack.getItem() == null) return
     * null; if(Block.getBlockFromItem(stack.getItem()) instanceof ILittleTile) { return
     * ((ILittleTile)Block.getBlockFromItem(stack.getItem())).getLittleTile(stack); } for (Entry<SortingTile,
     * LittleTile> entry : tiles.entrySet()) { SortingTile tile = entry.getKey();
     * if(Block.getBlockFromItem(stack.getItem()) == tile.block) { if(tile.meta == -1 || stack.getItemDamage() ==
     * tile.meta) return entry.getValue(); }else if(stack.getItem() == tile.customItemBlock){ if(tile.custommeta == -1
     * || stack.getItemDamage() == tile.custommeta) return entry.getValue(); } } return null; }
     */
