...
public class SubGuiStructure extends SubGui {
    ...
    public void createControls() {
        ...
        if (structure != null) {
            ...
            if (comboBox.index == -1) comboBox.index = 0;
            else comboBox.caption = structure.getIDOfStructure();
        }
    ...
    public void onChanged() {
        ...
        if (lastListener != null) removeListener(lastListener);

        LittleStructure saved = this.structure;
        if (saved != null && !saved.getIDOfStructure().equals(id)) saved = null;
        LittleStructure parser = null;
        LittleStructureEntry entry = LittleStructure.getEntryByID(id);
        if (entry != null) parser = entry.parser;
        if (parser != null) {
    ...
    public void onComboChange(ControlChangedEvent event) {
        if (event.source.is("types")) onChanged();
    }
