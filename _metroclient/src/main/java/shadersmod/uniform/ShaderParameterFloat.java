...
public enum ShaderParameterFloat implements IExpressionFloat {
      ...
      public float eval() {
            switch(this) {
            case BIOME:
                  Vec3 posCamera = Shaders.getCameraPosition();
                  BiomeGenBase biome = Shaders.getCurrentWorld().getBiomeGenForCoords((int)posCamera.xCoord, (int)posCamera.zCoord);
                  return (float)biome.biomeID;
            default:
                  Number valLegacy = LegacyUniforms.getNumber(this.name);
                  return valLegacy != null ? valLegacy.floatValue() : 0.0F;
            }
      }
}
